...
RSpec.describe Yodeler::Adapters::HttpAdapter do
  ...
  describe '#dispatch' do
    it 'adds the metric to the queue' do
      ...
      expect(WebMock).to have_requested(:post, 'http://example.com')
        .with(body: '{"uuid":"7ad1ef6a-e71c-4179-99e7-06c8f62151ce","name":"test","type":"gauge","value":35,"meta":{}}')
    end
  end
  ...
  describe '#handle' do
    ...
    context 'overwriting a specific metric handler' do
      it 'calls the handler when dispatched' do
        ...
        expect(WebMock).to have_requested(:post, 'http://example.com')
          .with(body: '{"uuid":"7ad1ef6a-e71c-4179-99e7-06c8f62151ce","name":"test.increment","type":"increment","value":1,"meta":{}}')

        expect(WebMock).to have_requested(:get, 'http://example.com')
          .with(body: '{"uuid":"7ad1ef6a-e71c-4179-99e7-06c8f62151ce","name":"test.gauge","type":"gauge","value":35,"meta":{}}')
      end
    end
  end
  ...
  describe '#default_params=' do
    it 'makes an HTTP request with the additional parameters' do
      ...
      expect(WebMock).to have_requested(:post, 'http://example.com')
        .with(body: '{"auth_token":"SECURZ","uuid":"7ad1ef6a-e71c-4179-99e7-06c8f62151ce","name":"test.gauge","type":"gauge","value":35,"meta":{}}')
    end
  end
